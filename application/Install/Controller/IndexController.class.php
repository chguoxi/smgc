...
class IndexController extends Controller {
    ...
    public function step4(){
        ...
        if(IS_POST){
            ...
            $sql = "DROP DATABASE IF EXISTS `msgc`; CREATE DATABASE IF NOT EXISTS `{$dbname}` DEFAULT CHARACTER SET utf8";
            //echo $sql;exit;
            $status = $db->execute($sql);
			if (!$status && !empty($db->getError())){
    ...
    public function step5(){
    	//dump(S("_install_step"));exit;
        if(S("_install_step")==4){
